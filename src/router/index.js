...
const mainRoutes = {
  ...
  children: [
    ...
    },
    {
      ...
      name: 'demo-test',
      meta: { title: 'demo-test', isTab: true }
    }
  ],
...
router.beforeEach((to, from, next) => {
  ...
  if (router.options.isAddDynamicMenuRoutes || fnCurrentRouteType(to, globalRoutes) === 'global') {
    ...
    getMenuList().then(({ data }) => {
      if (data && data.code === 0) {
        const newMenuData = filterMenuType(data.data.list, 2)
        fnAddDynamicMenuRoutes(newMenuData)
        router.options.isAddDynamicMenuRoutes = true
        sessionStorage.setItem('menuList', JSON.stringify(newMenuData || '[]'))
      ...
      if (errMsg.includes('401')) {
        ...
        console.log(`%c${error} 请求失败！`, 'color:blue')
      }
      // router.push({name: 'login'})
    })
  }
 ...
 * @returns {*}
 */
const filterMenuType = (menuList, menuType) => {
  return menuList.filter(item => {
    return (item.type) != menuType
  }).map(item => {
    item = Object.assign({}, item)
    if (item.list) {
      item.list = filterMenuType(item.list, menuType)
    }
    return item
  })
}
